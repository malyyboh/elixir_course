defmodule Distance do
  ...
  def poin_inside_figure?(point, {:circle, center, radius}) do
...
defmodule DistanceTest do
  ...
  test "distance crash" do
    ...
    assert_raise FunctionClauseError, fn -> distance(p, r) == 4.0 end
  end
  ...
  test "point inside circle" do
    ...
    assert poin_inside_figure?(p1, c)
    refute poin_inside_figure?(p2, c)
    assert poin_inside_figure?(p3, c)
    refute poin_inside_figure?(p4, c)
  end
  ...
  test "point inside figure crash" do
    ...
    assert_raise FunctionClauseError, fn -> poin_inside_figure?(p0, triangle) end
  end
end
